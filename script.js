...
function validarInscricao(inscricao) {
    ...
    const regex = /^\d{8}$/; // Valida 8 dígitos
    if (!regex.test(inscricao)) {
        return { valido: false, mensagem: "Formato inválido. Use exatamente 8 dígitos numéricos (Ex: 10000340)." };
    }
...
async function conferirRespostas() {
    ...
    // --- ADICIONADO: Limpar destaques anteriores ---
    for (let i = 1; i <= 60; i++) {
        const inputElement = document.getElementById(`q${i}`);
        if (inputElement) {
            inputElement.classList.remove('input-errado');
            // Não reseta a borda aqui ainda, a validação fará isso
        }
    }
    // --- FIM DA LIMPEZA ---
    ...
    inscricaoInput.style.borderColor = '#ccc';
    ...
    for (let i = 1; i <= 60; i++) {
        ...
        if (!input || !valor || !/^[A-E]$/.test(valor)) {
           ...
           if(input) input.style.borderColor = '#ccc'; // Reseta borda se válido
    ...
    // --- ADICIONADO: Destacar Respostas Erradas ---
    console.log("Destacando respostas erradas...");
    for (let i = 0; i < 60; i++) {
        const inputElement = document.getElementById(`q${i + 1}`); // Pega o input q1, q2, ...
        if (inputElement) { // Verifica se o input existe
            if (respostasUsuario[i] === gabaritoOficial[i]) {
                // Resposta CORRETA: Garante que a classe de erro seja removida
                inputElement.classList.remove('input-errado');
                // Não muda a borda aqui, pois já foi definida na validação
            } else {
                // Resposta ERRADA: Adiciona a classe de erro
                inputElement.classList.add('input-errado');
            }
        }
    }
    console.log("Destaque de erros concluído.");
    ...
    const submissionRef = db.collection("submissions").doc(inscricao);
...
function setupEventListeners() {
    if (submitBtn) {
        submitBtn.addEventListener('click', function() {
            ...
            conferirRespostas().catch(error => {
                 console.error("Erro não capturado por conferirRespostas:", error);
                 alert("Ocorreu um erro inesperado.");
                 if(submitBtn) { submitBtn.disabled = false; submitBtn.textContent = 'Conferir Respostas';}
            });
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (getDomElements()) { // Pega elementos primeiro
        setupEventListeners(); // Configura listeners depois
        if (typeof firebase !== 'undefined' && db && statsDocRef) {
            console.log("Firebase pronto. Chamando setupRealtimeUpdates...");
            setupRealtimeUpdates(); // Inicia listener do Firestore
        } else {
